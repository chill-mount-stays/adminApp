...
export const OrdersTableWithPopup: React.FC<OrdersTableWithPopupProps> = ({
  orders,
}) => {
          ...
          <TableBody>
            {currentOrders.map((order, idx) => (
              <TableRow key={idx}>
                ...
                <TableCell>
                  {order.stayItem && order.stayItem.length ? (
                    <Check
                      className="text-green-500"
                      aria-label="Stay included"
                    />
                  ) : (
                    <Minus
                      className="text-gray-300"
                      aria-label="Stay not included"
                    />
                  )}
                </TableCell>
                <TableCell>
                  {order.travelItem && order.travelItem.length ? (
                    <Check
                      className="text-green-500"
                      aria-label="Travel included"
                    />
                  ) : (
                    <Minus
                      className="text-gray-300"
                      aria-label="Travel not included"
                    />
                  )}
                </TableCell>
                <TableCell>
                ...
                <TableCell>
                  <Dialog>
                    ...
                    <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
                      ...
                      {selectedOrder && (
                        <CustomerOrderDisplay order={selectedOrder} />
                      )}
                    </DialogContent>
                  </Dialog>
                </TableCell>
